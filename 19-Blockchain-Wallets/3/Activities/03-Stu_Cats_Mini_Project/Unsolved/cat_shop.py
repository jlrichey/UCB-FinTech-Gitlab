...
# Create a for loop through the cat_database use `db_list to produce the results`
  ...
  # @TODO Use `st.write` to add the objects from the object to the code (hint price and name)
      ...
      st.write("Price in Ether: ", db_list[number][1], "ether")
      st.text(" \n")
...
st.markdown("# Kitties for Sale!!!!")
st.markdown("## Please buy me, you got money!!!!")
st.markdown("## Your girlfriend wants a kittie!!!")
st.text(" \n")
...
cat = st.sidebar.selectbox('Select your investment aka Kittie', kitties)
# @TODO: Create a header using ` st.sidebar.markdown()` to display cat name and price.
st.sidebar.markdown("## Kittie Name and Price")
