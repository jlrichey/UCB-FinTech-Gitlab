...
def pin_artwork(artwork_name, artwork_file):
    if artwork_file is None:
        st.warning("Please upload an artwork file.")
        return None, None

    # Pin the file to IPFS with Pinata
    ipfs_file_hash = pin_file_to_ipfs(artwork_file.getvalue())
    if not ipfs_file_hash:
        st.error("Failed to pin the artwork file to IPFS.")
        return None, None
    ...
    if not json_ipfs_hash:
        st.error("Failed to pin the artwork metadata to IPFS.")
        return None, None
...
if st.button("Register Artwork"):
    ...
    if artwork_ipfs_hash and token_json:
...
if st.button("Appraise Artwork"):
    ...
    if not appraisal_report_ipfs_hash:
        st.error("Failed to pin the appraisal report to IPFS.")
    else:
